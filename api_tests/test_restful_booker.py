"""
api_tests/test_restful_booker.py  v2.0 -- 2025 update
Added PATCH test, response time assertions, improved fixture isolation.
"""
...
class TestGetBookings:
    ...
    def test_filter_by_firstname(self, created_id, payload):
        ...
        assert created_id in [b["bookingid"] for b in r.json()]
...
class TestUpdateBooking:
    ...
    def test_update_without_token_returns_403(self, created_id, payload):
        assert requests.put(f"{BASE}/booking/{created_id}",
                            json=payload, headers=HEADERS).status_code == 403
...
class TestDeleteBooking:
    ...
    def test_delete_with_token(self, payload, token):
        ...
        assert requests.delete(f"{BASE}/booking/{bid}",
                               headers={**HEADERS, "Cookie": f"token={token}"}).status_code == 201
